...
//qrsh -g tga-hpc-lecture -l q_node=1 -l h_rt=2:00:00
...
int main(int argc, char** argv) {
  ...
  const int m = N/size, n = N/size, k = N/size;
  const int kc = 512;
  const int nc = 64;
  const int mc = 256;
  const int nr = 64;
...
#pragma omp parallel for collapse(2)
  for (int jc=0; jc<n; jc+=nc) {
    for (int pc=0; pc<k; pc+=kc) {
      ...
      for (int p=0; p<kc; p++) {
        for (int j=0; j<nc; j++) {
          Bc[p*nc+j] = subB[N/size*(p+pc)+(j+jc)];//B[p+pc][j+jc];
        }
      }
      for (int ic=0; ic<m; ic+=mc) {
	    float Ac[mc*kc],Cc[mc*nc];
          for (int i=0; i<mc; i++) {
            for (int p=0; p<kc; p++) {
              Ac[i*kc+p] = subA[N*(i+ic)+(p+pc)];//A[i+ic][p+pc];
            }
            for (int j=0; j<nc; j++) {
              Cc[i*nc+j] = 0;
            }
          ...
          for (int i=0; i<mc; i++) {
            for (int j=0; j<nc; j++) {
              subC[N*(i+ic)+(j+jc)+offset] += Cc[i*nc+j];//C[i+ic][j+jc]->subC
            }
          }
        }
      }
